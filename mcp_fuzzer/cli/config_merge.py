...
# Config keys that may override an argparse field left at its default. The
# argparse destination has the same name unless listed in _ARGS_KEY_OVERRIDES.
_TRANSFERABLE_CONFIG_KEYS = (
    "endpoint",
    "protocol",
    "mode",
    "phase",
...
def _transfer_config_to_args(args: argparse.Namespace) -> None:
    ...
    for config_key in _TRANSFERABLE_CONFIG_KEYS:
        args_key = _ARGS_KEY_OVERRIDES.get(config_key, config_key)
...
def build_cli_config(args: argparse.Namespace) -> CliConfig:
    ...
    merged: dict[str, Any] = {
        ...
        "runtime_probe_backend": getattr(args, "runtime_probe_backend", None),
        "runtime_probe_bin": getattr(args, "runtime_probe_bin", None),
        "runtime_probe_workspace": getattr(args, "runtime_probe_workspace", None),
        "runtime_probe_tmpdir": getattr(args, "runtime_probe_tmpdir", None),
        "runtime_probe_allow_exec": getattr(args, "runtime_probe_allow_exec", None),
        "runtime_probe_allow_host": getattr(args, "runtime_probe_allow_host", None),
