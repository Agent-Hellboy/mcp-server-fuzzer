...
def _write_counts_section(
    ...
    writer.write("-" * 40 + "\n")
    writer.write(_format_counts(counts) + "\n\n")


def _write_security_summary(writer, summary: dict[str, dict[str, int]] | None) -> None:
    if not summary:
        return
    ...
    writer.write("-" * 40 + "\n")
    _write_counts_section(writer, "Oracle findings by type", oracle_counts)
    _write_counts_section(writer, "Policy violations by domain", domain_counts)
    _write_counts_section(writer, "Policy controls", control_counts)
...
class TextFormatter:
    ...
    def save_text_report(
        ...
        with open(filename, "w", encoding="utf-8") as f:
            ...
            _write_security_summary(f, security_summary)
