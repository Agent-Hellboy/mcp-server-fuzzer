...
The exact files depend on the run and the selected `--output-types`. The
standardized writer emits JSON. Additional human-facing exports use
`--export-csv`, `--export-xml`, `--export-html`, or `--export-markdown`.
...
Session IDs and JSON file names are generated by the writer. Report format,
compression, and a custom schema path are configured under the nested `output:`
section of a YAML config file (`format`, `compress`, `schema`), not on the
command line.
