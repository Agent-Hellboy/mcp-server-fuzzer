...
@pytest.mark.asyncio
async def test_handle_server_request_sampling_create_message(monkeypatch):
...
@pytest.mark.asyncio
async def test_receive_message_error_resets_state(monkeypatch):
    transport = StdioDriver("echo")
...
@pytest.mark.asyncio
async def test_send_request_no_response(monkeypatch):
    transport = StdioDriver("echo")
    transport._initialized = True
    transport._send_message = AsyncMock()
...
@pytest.mark.asyncio
async def test_send_request_server_error(monkeypatch):
    transport = StdioDriver("echo")
    transport._initialized = True
    transport._send_message = AsyncMock()
    monkeypatch.setattr(
        transport.send_request.__globals__["uuid"],
        "uuid4",
        lambda: "req",
    )
...
@pytest.mark.asyncio
async def test_send_request_initialize_updates_spec(monkeypatch):
    transport = StdioDriver("echo")
    transport._initialized = True
    transport._send_message = AsyncMock()
    ...
    seen = {}
    monkeypatch.setitem(
        transport.send_request.__globals__,
...
@pytest.mark.asyncio
async def test_send_request_exhausts_iterations(monkeypatch):
    transport = StdioDriver("echo")
    transport._initialized = True
    transport._send_message = AsyncMock()
...
async def test_send_raw_initialize_updates_spec(monkeypatch):
    ...
    seen = {}
    monkeypatch.setitem(
        transport.send_raw.__globals__,
        "maybe_update_spec_version_from_result",
        lambda result: seen.setdefault("pv", result.get("protocolVersion")),
    )
...
@pytest.mark.asyncio
async def test_send_timeout_signal_unknown_direct(monkeypatch):
    transport = StdioDriver("echo")
...
@pytest.mark.asyncio
async def test_send_timeout_signal_registered(monkeypatch):
    transport = StdioDriver("echo")
    transport.process = SimpleNamespace(pid=123)
    transport.manager = SimpleNamespace(state=MagicMock(), emit_event=MagicMock())
...
@pytest.mark.asyncio
async def test_ensure_connection_fallback_kill(monkeypatch):
    transport = StdioDriver("echo")
    transport._initialized = True
...
@pytest.mark.asyncio
async def test_send_request_reads_json(monkeypatch):
    transport = StdioDriver("echo")
...
@pytest.mark.asyncio
async def test_send_request_no_line(monkeypatch):
    transport = StdioDriver("echo")
...
@pytest.mark.asyncio
async def test_send_request_awaitable_line(monkeypatch):
    transport = StdioDriver("echo")
...
@pytest.mark.asyncio
async def test_stream_request_skips_invalid_json(monkeypatch):
    transport = StdioDriver("echo")
...
@pytest.mark.asyncio
async def test_stream_request_bytes(monkeypatch):
    transport = StdioDriver("echo")
...
@pytest.mark.asyncio
async def test_stream_request_awaitable_line(monkeypatch):
    transport = StdioDriver("echo")
...
@pytest.mark.asyncio
async def test_close_registers_and_waits(monkeypatch):
    transport = StdioDriver("echo")
...
@pytest.mark.asyncio
async def test_close_fallback_without_pid(monkeypatch):
    transport = StdioDriver("echo")
...
@pytest.mark.asyncio
async def test_send_timeout_signal_timeout_direct(monkeypatch):
    transport = StdioDriver("echo")
...
@pytest.mark.asyncio
async def test_send_timeout_signal_force_direct(monkeypatch):
    transport = StdioDriver("echo")
    transport.process = SimpleNamespace(
        pid=321,
        terminate=MagicMock(),
...
@pytest.mark.asyncio
async def test_send_timeout_signal_interrupt_direct(monkeypatch):
    transport = StdioDriver("echo")
    transport.process = SimpleNamespace(
        pid=321,
        terminate=MagicMock(),
...
@pytest.mark.asyncio
async def test_send_timeout_signal_direct_failure(monkeypatch):
    transport = StdioDriver("echo")
