...
class TestCheckToolResultContent:
    ...
    def test_text_content_missing_text(self):
        ...
        assert any(c["check_id"] == "tools-content-text" for c in result)
    ...
    def test_image_content_missing_data(self):
        ...
        result = check_tool_result_content({"content": [{"type": "image", "mimeType": "image/png"}]})
        assert any(c["check_id"] == "tools-content-image-data" for c in result)

    def test_image_content_missing_mimetype(self):
        """Test image content missing mimeType field."""
        result = check_tool_result_content({"content": [{"type": "image", "data": "base64..."}]})
        assert any(c["check_id"] == "tools-content-image-mime" for c in result)

    def test_audio_content_missing_data(self):
        """Test audio content missing data field."""
        result = check_tool_result_content({"content": [{"type": "audio", "mimeType": "audio/mp3"}]})
        assert any(c["check_id"] == "tools-content-audio-data" for c in result)

    def test_audio_content_missing_mimetype(self):
        """Test audio content missing mimeType field."""
        result = check_tool_result_content({"content": [{"type": "audio", "data": "base64..."}]})
        assert any(c["check_id"] == "tools-content-audio-mime" for c in result)

    def test_resource_content_non_dict_resource(self):
        """Test resource content with non-dict resource."""
        result = check_tool_result_content({"content": [{"type": "resource", "resource": "not a dict"}]})
        assert any(c["check_id"] == "tools-content-resource" for c in result)

    def test_resource_content_missing_uri(self):
        """Test resource content missing uri."""
        result = check_tool_result_content({
            "content": [{"type": "resource", "resource": {"mimeType": "text/plain", "text": "data"}}]
        })
        assert any(c["check_id"] == "tools-content-resource-uri" for c in result)

    def test_resource_content_missing_mimetype(self):
        """Test resource content missing mimeType."""
        result = check_tool_result_content({
            "content": [{"type": "resource", "resource": {"uri": "file://test", "text": "data"}}]
        })
        assert any(c["check_id"] == "tools-content-resource-mime" for c in result)

    def test_resource_content_missing_body(self):
        """Test resource content missing text or blob."""
        result = check_tool_result_content({
            "content": [{"type": "resource", "resource": {"uri": "file://test", "mimeType": "text/plain"}}]
        })
        assert any(c["check_id"] == "tools-content-resource-body" for c in result)

    def test_unknown_content_type(self):
        """Test unknown content type generates warning."""
        result = check_tool_result_content({"content": [{"type": "custom_type"}]})
        assert any(c["check_id"] == "tools-content-unknown-type" for c in result)
    ...
    def test_is_error_without_text_message(self):
        ...
        assert any(c["check_id"] == "tools-error-text" for c in result)
    ...
    def test_is_error_with_text_message(self):
        ...
        assert not any(c["check_id"] == "tools-error-text" for c in result)
...
class TestCheckResourceTemplatesList:
    ...
    def test_non_array_templates(self):
        ...
        result = check_resource_templates_list({"resourceTemplates": "not an array"})
        assert len(result) == 1
        assert result[0]["check_id"] == "resources-templates-type"

    def test_non_dict_template_item(self):
        """Test template item that is not a dict."""
        result = check_resource_templates_list({"resourceTemplates": ["not a dict"]})
        assert any(c["check_id"] == "resources-templates-item" for c in result)

    def test_template_missing_uri(self):
        """Test template missing uri."""
        result = check_resource_templates_list({"resourceTemplates": [{"name": "test"}]})
        assert any(c["check_id"] == "resources-templates-uri" for c in result)

    def test_valid_templates(self):
        """Test valid templates."""
        result = check_resource_templates_list({"resourceTemplates": [{"uri": "file://template"}]})
        assert result == []
