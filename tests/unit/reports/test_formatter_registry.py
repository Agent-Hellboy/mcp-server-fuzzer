...
def test_formatter_registry_saves(tmp_path):
    ...
    registry.register("txt", FormatterAdapter(save_fn, "txt"))
    snapshot = _snapshot()
...
def test_formatter_adapter_save_with_filename(tmp_path):
    ...
    adapter = FormatterAdapter(save_fn, "json")
    snapshot = _snapshot()
...
def test_formatter_adapter_save_with_absolute_filename(tmp_path):
    ...
    adapter = FormatterAdapter(save_fn, "txt")
    snapshot = _snapshot()
...
def test_html_formatter_adapter_save_uses_title(tmp_path):
    ...
    adapter = HtmlFormatterAdapter(save_fn, title="Custom Title")
    snapshot = _snapshot()
