...
def test_apply_config_file_handles_load_errors(config_files):
    ...
    with (
        patch("mcp_fuzzer.config.loader.config") as mock_config,
        patch("mcp_fuzzer.config.loader.load_config_file") as mock_load_config,
        patch("mcp_fuzzer.config.loader.load_custom_transports") as mock_load_custom,
    ):
